...
ggplot() + 
  ...
  labs(x = "Period", y = "Price", color = "Legend") +
  scale_colour_manual(name="Keterangan:", breaks = c("Data Latih", "Data Uji"),
                      values = c("blue", "red")) + 
  theme_bw() + theme(legend.position = "bottom",
                     plot.caption = element_text(hjust=0.5, size=12))
...
```{r}
#Cara 1 (fungsi ses)
ses.1 <- ses(train.ts, h = 40, alpha = 0.2)
plot(ses.1)
ses.1

ses.2<- ses(train.ts, h = 40, alpha = 0.7)
plot(ses.2)
ses.2
```
...
#ramalan
ramalanopt<- forecast(HWopt, h=40)
ramalanopt
```
...
#ramalan
ramalandes1<- forecast(des.1, h=40)
ramalandes1
...
#ramalan
ramalandes2<- forecast(des.2, h=40)
ramalandes2
```
...
#ramalan
ramalandesopt<- forecast(des.opt, h=40)
ramalandesopt
```
